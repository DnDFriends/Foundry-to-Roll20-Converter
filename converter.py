    ...
    def __init__(self, actor, spell_ability=None, overrides=None, review=None):
        ...
        if detected not in ABILITIES:
            ...
            if len(candidates)==1:
                ...
                detected = max(sorted(candidates),key=lambda ability:self.mods.get(ability,-99))
                self.warn(f'Multiple class spellcasting abilities were found; Auto-detect selected {ABILITIES[detected].title()}. Choose an ability manually if this character uses a different primary ability.')
        if detected not in ABILITIES:
            ...
            if len(candidates)==1:
                ...
                detected = max(sorted(candidates),key=lambda ability:self.mods.get(ability,-99))
                self.warn(f'Multiple spell-item abilities were found; Auto-detect selected {ABILITIES[detected].title()}. Choose an ability manually if this character uses a different primary ability.')
        if detected not in ABILITIES and any(i['type']=='spell' for i in self.items):
            detected = max(('int','wis','cha'),key=lambda ability:self.mods[ability])
            self.warn(f'Foundry did not designate a spellcasting ability; Auto-detect selected the character\'s highest mental ability, {ABILITIES[detected].title()}. Choose an ability manually if needed.')
